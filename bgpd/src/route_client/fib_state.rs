...
#[derive(Debug)]
pub struct FibEntry<A> {
    nexthop: A,
}

pub struct FibState<A: Address> {
    pub fib: IpLookupTable<A, Arc<Mutex<FibEntry<A>>>>,
    pub nl_iface: NetlinkInterface,
    pub af: u8,
    pub table: u32,
}

impl<A: Address> std::fmt::Debug for FibState<A> {
    ...
    String: From<<A as TryFrom<NLRI>>::Error>,
{
    ...
    pub fn route_add(&mut self, nlri: &NLRI, nexthop: A) -> Result<(), String> {
        ...
        {
            Some(entry_wrapped) => {
                let mut entry = entry_wrapped.lock().unwrap();
    ...
    #[tracing::instrument]
    pub fn route_del(&mut self, nlri: NLRI) -> Result<(), String> {
        let prefix_addr: A = nlri.clone().try_into()?;
